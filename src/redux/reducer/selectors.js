...
export const exporterSelector = createSelector(
  ...
  idSelector,
  (importItems, id) => {
    const buff = importItems.find((exporter) => exporter.id === id);
    return buff;
  },
);

export const orderBoxSelector = createSelector(
  productsSelector,
  orderSelector,
  (products, order) => {
...
export const experienceIdSelector = createSelector(
  experienceSelector,
  ownPropsExperienceIdSelector,
...
export const activeExporterSelector = (state, activeExporterId) => {
  return state.allExporters.entities.find((exporter) => activeExporterId === exporter.id);
};
